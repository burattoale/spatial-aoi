...
def do_simulation_pair(params:SimulationParameters, sim_length:int):
    ...
    time_evolution_loc_aware_mc, mean_e_hmm_loc_aware_mc, _ = run_hmm_simulation(params, num_time_steps=sim_length, loc_aware=True)
    ...
    return mean_e_forgetful, mean_e_hmm, mean_e_hmm_loc_aware, mean_e_hmm_loc_aware_mc
...
if __name__ == "__main__":
    ...
    res = Parallel(n_jobs=parallel_jobs, backend="loky", verbose=1)(
        delayed(parallel_k)(k, params, sim_length, n_topologies, parallel_jobs) for k in range(5, k_max, 2)
    )
    ...
    plt.figure()
    plt.semilogy(np.arange(5,k_max,2)*R, entropies_k_forgetful, marker='.', label="Forgetful receiver")
    plt.semilogy(np.arange(5,k_max,2)*R, entropies_k_hmm, marker='.', label="HMM receiver")
    plt.semilogy(np.arange(5,k_max,2)*R, entropies_k_hmm_loc_aware,linestyle=":", marker='.', label="HMM receiver loc aware")
    plt.semilogy(np.arange(5,k_max,2)*R, entropies_k_hmm_loc_aware_mc,linestyle=":", marker='.', label="HMM receiver loc aware Monte Carlo")
